    ...
    <div className="flex items-end gap-[3px] h-[80px] w-full">
      {data.map((d, i) => {
        ...
        return (
          <div
            key={i}
            className="flex items-end gap-[1px] flex-1 group relative"
          >
            <div
              className="flex-1 bg-primary/20 rounded-sm"
              style={{ height: sh }}
              title={`Sent: ${formatNumber(d.sent)}`}
            />
            <div
              className="flex-1 bg-primary/50 rounded-sm"
              style={{ height: dh }}
              title={`Delivered: ${formatNumber(d.delivered)}`}
            />
            <div
              className="flex-1 bg-primary rounded-sm"
              style={{ height: rh }}
              title={`Read: ${formatNumber(d.read)}`}
            />
          </div>
        );
      })}
                ...
                >
                  ...
                  <div className="w-full h-1 bg-muted rounded-full overflow-hidden">
                    <div
                      className={cn(
                        "h-full rounded-full",
                        card.up ? "bg-emerald-500" : "bg-red-400",
                      )}
                      style={{ width: "65%" }}
                    />
                  </div>
                      ...
                      >
                        <div className="flex items-center gap-1.5">
                          <span
                            ...
                            style={{ background: DONUT_COLORS[i] ?? "#94a3b8" }}
                          />
                ...
                <div className="space-y-3">
                  {(analytics?.topTemplates ?? []).map((t, i) => {
                    const maxSent = Math.max(
                      ...(analytics?.topTemplates ?? []).map((x) => x.sent),
                    );
                    return (
